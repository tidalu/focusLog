...
import {
  buildReminderOverlayControllerScript,
  buildReminderOverlayHtml
} from './reminders/reminder-overlay-html.js';
...
function createReminderOverlay(occurrenceId: string): void {
  ...
  const executableHtml = buildReminderOverlayHtml(occurrenceId, intervalMinutes) || html;
  void overlay.loadURL(`data:text/html;charset=UTF-8,${encodeURIComponent(executableHtml)}`);
  overlay.webContents.once('did-finish-load', () => {
    void overlay.webContents
      .executeJavaScript(buildReminderOverlayControllerScript(occurrenceId), true)
      .catch(() => undefined);
  });
